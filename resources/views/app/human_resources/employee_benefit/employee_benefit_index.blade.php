...
@section('content')
    <div class="mx-auto max-w-7xl px-4 sm:px-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h1 class="text-xl font-semibold">Benefícios por funcionário</h1>
        </div>

        <!-- busca + ações na mesma linha -->
        <div class="mt-4 flex items-center gap-4">
            <div class="relative w-full max-w-xl">
                <input id="search" placeholder="Buscar por observações..."
        ...
        <div class="mt-5 mb-8 rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div>
                <table class="min-w-full text-sm">
                    ...
                    <tr>
                        <th class="px-6 py-4 first:rounded-tl-2xl text-left">Funcionário</th>
                        <th class="px-3 py-4 text-left">Benefício</th>
                        <th class="px-3 py-4 text-right">Valor (R$)</th>
                        <th class="px-3 py-4 text-left">Observações</th>
    ...
    @include('app.human_resources.employee_benefit.employee_benefit_modal')
    @include('layouts.common.modal.modal_delete')
@endsection

@push('scripts')
    <script type="module" src="{{ asset('assets/js/template/views/human-resources/employee-benefit.js') }}"></script>
@endpush
