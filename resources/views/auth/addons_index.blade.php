...
<script>
    ...
    function renderCards() {
        ...
        CATALOG.forEach((m) => {
            ...
            const previewDesc =
                fullDesc.length > 40 ? fullDesc.slice(0, 40) + "…" : fullDesc;
            ...
            card.innerHTML = `
        <span class="pointer-events-none absolute inset-0 rounded-2xl ring-2 ring-blue-600 ring-offset-0 ${
                checked ? "" : "hidden"
            } peer-checked:block"></span>
        ...
        <div class="flex items-start gap-3 relative z-[1]">
          ...
          <div class="text-right pt-7">
            <div class="text-sm font-semibold text-slate-900">${brl(
                Number(m.price)
            )}</div>
        ...
        <div class="pointer-events-none absolute right-3 top-3 ${
                checked ? "" : "hidden"
            } h-6 w-6 rounded-full bg-blue-600 text-white shadow flex items-center justify-center peer-checked:flex">
    ...
    function renderChips() {
        ...
        [...selected].forEach((id) => {
            ...
            chip.type = "button";
            chip.className =
                "inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-800 hover:bg-blue-100";
            chip.innerHTML = `
                ${m.name}${
                m.required
                    ? ' <span class="opacity-60">(obrigatório)</span>'
                    : ""
            }
    ...
    function renderSummary() {
        ...
        [...selected].forEach((id) => {
            ...
            li.innerHTML = `
                <span>${m.name}${
                m.required
                    ? ' <span class="ml-1 text-[10px] px-1 rounded bg-slate-100 text-slate-600">obrigatório</span>'
                    : ""
            }</span>
        ...
        noteEl.textContent =
            "deverá ser pago para continuar utilizando o sistema com as funcionalidades escolhida.";

        const has = selected.size > 0;
        if (continueBtn) continueBtn.disabled = !has;
        if (continueMob) continueMob.disabled = !has;
