...
{
    use CreateCustomerAsaas, RoleCheckTrait, TenantPermissionUser;
    ...
    public function additionalInfoIndexStore(StoreAdditionalCustomerInfoRequest $request)
    {
        ...
        if ($existingLogin) {
            ...
            // Atualiza dados básicos do Customer
        ...
        $this->ensureTenantBasePermissions($customerSistappId);
    ...
    {
        $request->validate([
            'segment' => 'required|in:authorized',
        ]);

        $user = auth()->user();
        $customerSistappId = optional($user->customerLogin)->customer_sistapp_id;
